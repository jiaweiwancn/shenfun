...
For the nominal `Re_tau=395`, `(192,256,192)` all-plane temporal-sampling
workflow on the CentOS/SLURM server, start with:

```text
RUN_PLAN_mkm_Re_tau_395.md
TEMPORAL_SAMPLING_REMEDY_Re_tau_395.md
re395_server/README.md
```

That package uses a gated small even-grid round trip before any full-shape or
production submission. Its dense temporal record stores the two independent
MKM spectral state variables and required zero modes, not selected
wall-normal planes.

The legacy helper commands below use the cloned repository on the earlier
data-backed Linux environment:
